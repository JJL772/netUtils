...
static void
_probe_one(struct probe_opts_s* probe_opts, int cur_addr, struct probe_result_s* result)
{
  ...
  defpopts.log_type = probe_opts->verbose  ? PING_LOG_FULL
                      : probe_opts->sentry ? PING_LOG_NONE
                                           : PING_LOG_MINIMAL;
...
#define NUM_SAMPLES 10
  ...
  while (1) {
    for (int i = 0; i < probe_opts->tries; ++i) {
      ...
      if (!probe_opts->sentry)
        printf(
          "  Completed (pattern 0x%X, size %u): %d sent, %d lost, %d corrupted, maxTime %f, "
          "minTime %f, avgTime %f\n",
          (int)popts.pattern,
          size,
          pstat.sent,
...
void
probe_opts_init(struct probe_opts_s* opts)
{
...
static void
probe(struct probe_opts_s* opts)
{
...
static void
probe_iocsh(const iocshArgBuf* buf)
{
  probe_cmd(buf[0].aval.ac, buf[0].aval.av);
}

static void
probe_kill(const iocshArgBuf* buf)
{
  if (!s_thread)
    return;
  s_threadRun = 0;
