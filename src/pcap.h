...
extern int pcap_add_packet(
  pcap_file_t* file, pcap_timestamp_t ts, const void* data, int64_t datalen, int64_t orglen
);

inline static int
pcap_add_packet_now(pcap_file_t* file, const void* data, int64_t datalen, int64_t orglen)
{
...
pcap_file_t*
pcap_file_create(const char* path, int link_layer_type)
{
...
void
pcap_file_close(pcap_file_t* file)
{
  if (file->fp)
    fclose(file->fp);
  free(file);
...
void
pcap_file_flush(pcap_file_t* file)
{
  if (file->fp)
    fflush(file->fp);
}
