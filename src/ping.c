...
static bool
_icmp_validate(const struct ping_opts* opts, struct ping_packet* packet, ssize_t recv_size);
static void _generate_packet(
  const struct ping_opts* opts, struct ping_packet* packet, uint16_t seq, uint16_t ident
);
...
static void
iocsh_ping(const iocshArgBuf* args)
{
...
static bool
_ping_open(in_addr_t addr, const struct ping_opts* opts, struct ping_ctx* p)
{
#ifdef USE_RAW_SOCK
...
ping_help()
{
  printf("Usage: ping [-c count] [-i interval] [-s payload size] [-p pattern] [-l progress "
         "interval] [-q] ADDR\n");
}

void
icmp_ping_opts_init(struct ping_opts* opts)
{
...
bool
icmp_ping_cmd(int argc, char** argv)
{
  ...
  printf(
    "PING %s %d (%zu) bytes of data, pattern %d\n",
    inet_ntoa(a),
    opts.payload_size,
    opts.payload_size + sizeof(struct ping_packet),
    opts.pattern
  );
...
bool
icmp_ping(const struct ping_opts* opts, struct ping_stats* stats)
{
  ...
  recvagain:
    while (1) {
      ...
      // Recv logic is a bit more convoluted when using SOCK_RAW. We recv a full IP frame instead of
      // just the payload
      struct ip* ipf = (struct ip*)m.raw;

      const size_t recv_size = packet_size + sizeof(struct ip);
      if ((ret = recvfrom(ctx.fd, m.raw, recv_size, 0, (struct sockaddr*)&fromaddr, &fromsize)) <
          (ssize_t)(sizeof(struct ping_packet))) {
      ...
      // Certain servers may be configured to truncate ICMP requests above a certain size (i.e.
      // google.com)
      ...
      if (!quiet)
        printf(
          "%ld bytes from %s: icmp_seq=%d time=%.2f ms %s%s%s\n",
          (long int)(ret - ipf->ip_hl * 4),
    ...
    if (!silent && quiet && opts->progress > 0 && seq % opts->progress == 0) {
      printf(
        "%llu sent so far, %lli in-flight (or lost), %lli corrupted, icmp_seq=%llu\n",
        (long long unsigned)seq + 1,
        (long long)stats->lost,
        (long long)stats->corrupted,
  ...
  if (!silent) {
    printf(
      "%llu packets transmitted, %llu received, %lld corrupted, %.2f%% packet loss\n",
      (long long unsigned)seq,
      (long long unsigned)seq - stats->lost,
      (long long)stats->corrupted,
      100.f * ((float)(stats->lost) / seq)
    );
    printf(
      "min=%.2f ms, max=%.2f ms, avg=%.2f ms\n", stats->minTime, stats->maxTime, stats->avgTime
    );
...
_icmp_validate(const struct ping_opts* opts, struct ping_packet* packet, ssize_t recv_size)
{
  ...
  if (sum != actualSum) {
    printf(
      "bad checksum for icmp_seq=%d: packet checksum=0x%X, expected checksum=0x%X\n",
      packet->icmp.icmp_hun.ih_idseq.icd_seq,
      sum,
      actualSum
    );
  }
  ...
  for (int i = 0; i < toCheck; ++i) {
    if ((char)opts->pattern != packet->payload[i]) {
      printf(
        "Data failed to validate, expected %d at byte %d, but got %d insteaad!\n",
        opts->pattern,
        i,
        packet->payload[i]
      );
...
static void
_generate_packet(
  const struct ping_opts* opts, struct ping_packet* msg, uint16_t seq, uint16_t ident
)
{
